...
**結論**:
- **FEX は 18/20 ワークロードで QEMU より高速** — 全カテゴリで優位（node は Tie、rustc は QEMU SIGSEGV）
...
- **Node.js / .NET / gcc / make が全て安定動作** — 旧ビルド (未最適化) でのクラッシュ・不安定問題は解消
- **libkrun 環境では FEX が唯一の高速エミュレーション選択肢** — Rosetta は applehv 専用のため使用不可

> **⚠️ 前回 (2026-04-05) からの主な変更**: `-DCMAKE_BUILD_TYPE=Release` 適用で FEX 全体が 2〜54x 高速化。
> Node.js (exit 132/SIGILL)・.NET (exit 139/SIGSEGV) のクラッシュが解消、gcc/make の反復不安定も消失。
> **⚠️ Rosetta 再計測 (2026-04-06)**: 前回は provider 切替ミスで libkrun/QEMU で計測されていた。今回 applehv を明示設定し正しい Rosetta 数値を取得。
...
|--:|----------|--------:|---------:|------------:|:------:|:-----------:|:------|
| 18 | `java HelloWorld` | 526 | 623 | **421** | **Rosetta** | 1.25x | FEX: 10/10 ok |
| 19 | `node -e console.log(42)` | 461 | 468 | **311** | **Rosetta** | 1.48x | FEX: 10/10 ok (旧: exit 132) |
| 20 | `dotnet --info` | **561** | 1,682 | 908 | **FEX** | 0.62x | FEX: 10/10 ok (旧: exit 139) |
...
| **QEMU wins** | **0** | — |
| **特記** | **1** | rustc — FEX=1,490ms(ok) / QEMU=exit 139(SIGSEGV) |

## Key Insights

### 1. Release ビルドで FEX が大半のワークロードで Rosetta を上回る

`-DCMAKE_BUILD_TYPE=Release` 適用により、FEX は 13/20 ワークロードで Rosetta を上回った。
特に interpreter startup (python3 0.37x, perl 0.31x) や package manager (dpkg 0.32x, dnf 0.53x) で圧倒的。
...
旧データでの問題:
- Node.js (exit 132 / SIGILL) → **10/10 ok** (461ms)
- .NET (exit 139 / SIGSEGV) → **10/10 ok** (561ms)
- gcc / make: 3 回目以降 exit 4/2 → **10/10 ok** (201ms / 227ms)

Release ビルドにより最適化フラグ (`-O3`) が適用され、JIT コード生成の品質が向上。
未最適化ビルドでの不具合は最適化不足による JIT コード品質の問題だったと推定される。
