...
| **キャッシュ方式** | コンテナ内エフェメラル (`/tmp/fex-emu/cache/`) |
...
FEX-Emu のコードキャッシュは **コンテナ内** (`/tmp/fex-emu/cache/`) に保存される。
...
| 変数 | 値 |
|------|-----|
| `FEX_APP_CACHE_LOCATION` | `/tmp/fex-emu/cache/` |
| `FEX_APP_CONFIG_LOCATION` | `/tmp/fex-emu/` |
| `FEX_APP_DATA_LOCATION` | `/tmp/fex-emu/` |
