#!/bin/bash
# Issue #27817: Fedora shell hang under Rosetta
# https://github.com/containers/podman/issues/27817
set +e
...
echo "=== TEST: ${TEST_NAME} ==="
echo "Issue: #27817 - Fedora shell hangs under Rosetta"
echo ""
...
pcmd run --rm --name "$CN" \
  --arch amd64 "${IMAGE}" \
  bash -c 'echo "Fedora running successfully" && uname -a && cat /etc/os-release' 2>&1
