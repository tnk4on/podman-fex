#!/bin/bash
# Issue #27210: Arch Linux hang under Rosetta
# https://github.com/containers/podman/issues/27210
set +e

TEST_NAME="06-archlinux"
TIMEOUT=30
CN="fex-${TEST_NAME}-$$"
...
echo "=== TEST: ${TEST_NAME} ==="
echo "Issue: #27210 - Arch Linux container hangs"
echo ""
...
pcmd run --rm --name "$CN" \
  --arch amd64 "${IMAGE}" \
  bash -c 'echo "Arch Linux running successfully" && uname -a && cat /etc/os-release' 2>&1
